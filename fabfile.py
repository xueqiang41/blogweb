...
db_user = 'blogwebuser'
db_password = 'blogwebuser'

_TAR_FILE = 'dist-blogweb.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/home/xueqiang/srv/blogweb'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-blogweb-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick blogweb > %s' % (db_user, db_password, f))
...
def deploy():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        sudo('chown -R xueqiang:xueqiang %s' % newdir)
        sudo('chown xueqiang:xueqiang www')
    with settings(warn_only=True):
        sudo('supervisorctl stop blogweb')
        sudo('supervisorctl start blogweb')
